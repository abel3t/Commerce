...
namespace Commerce.Domain.Repository
{
    public abstract class BaseRepository<T> : IRepository<T> where T : class
    {
        ...
        private FilterDefinition<T> Id(object value) => Builders<T>.Filter.Eq("_id", ObjectId.Parse(value.ToString()));
        ...
        public virtual void AddUow(T entity)
        {
            Context.AddCommand(() => Collection.InsertOneAsync(entity));
        }

        public virtual void UpdateUow(object id, T entity)
        {
            Context.AddCommand(() =>
                Collection.ReplaceOneAsync(Id(id), entity, new ReplaceOptions() { IsUpsert = false }));
        }

        public virtual void RemoveUow(object id)
        {
            Context.AddCommand(() => Collection.DeleteOneAsync(Id(id)));
        }
        ...
        public T FindOneAndUpdate(FilterDefinition<T> filter, UpdateDefinition<T> update,
        ...
        public virtual void FindOneAndDelete(object id)
        {
            var x = Collection.DeleteOne(Id(id));
        }
